...
As forças envolvidas para erguer a carga e fazer o deslocamento do tubo de Pitot na horizontal 
são baixas, não havendo necessidade de se fazer um cálculo estrutural, porém por se tratar de 
um projeto mecânico, é inevitável de que se tenha a garantia de que a estrutura resista ao 
esforços mecânicos. Para tanto será feito um cálculo para obter valores máximos de esforços 
e compará-los com os valores admissíveis.

Considerando apoios simples com carga central.

Dados: $E_{Aco} = 190~GPa$, $D_{haste} = 16~mm$, $L = 400~mm$.
...
A equação \ref{eq:momentodeinercia} calcula o momento de inercia.
...
$$I_{f} = \frac{\pi \cdot (7~mm)^{4}}{64} = 117,8588119~mm^{4}$$ 
...
Utilizando a equação \ref{eq:deflexao} se obtem a força considerando $Y_{max} = 0,05~mm$ e $I_{h}$:
