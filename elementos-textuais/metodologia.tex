...
Os cálculos a seguir serão fundamentados nos livros Juvinall 5° edição e Shigley 8ª edição.

\subsubsection{Cálculo da máxima deflexão}

Dados: 
$E_{Aco}~=~190~GPa$ de acordo com o apêndice C1 do Juvinall 5° edição,\\
...
A equação \ref{eq:momentodeinercia} calcula o momento de inércia segundo o apêndice B1a do Juvinall 5° edição.
...
A equação \ref{eq:deflexao} calcula a deflexão segundo tabela A9 do Shigley 8ª edição.
...
\begin{itemize}
    ...
    \item $P$ é o passo;
    \item $L$ é o avanço que é igual ao número de entradas do passo;
    \item $T$ é o torque do motor (Datasheet);
\end{itemize}

A equação \ref{eq:dmedio} calcula o diâmetro médio segundo a tabela xx do Shigley 8ª edição.
...
\begin{equation}\label{eq:avanco}
    L = Numero de entradas \cdot passo = 8~mm
\end{equation}

Utilizando a equação \ref{eq:avanco} se obtém o avanço considerando o número de entrada igual a 4 e o passo igual $2~mm$:
$$L = 4 \cdot 2 = 8~mm$$ 

Considerando o torque do motor conforme o datasheet.
$$T = 5~kgf \cdot cm = 490,33 N \cdot mm$$

\begin{equation}\label{eq:torque}
T = \frac{F \cdot d_{m}}{2} \cdot (\frac{L + \pi \cdot f \cdot d_{m}}{\pi \cdot d_{m} - f \cdot L}) + \frac{F \cdot f_{c} \cdot d_{c}}{2}
\end{equation}
